...
namespace BlazorServer2
{
    public static class BadWordsHelper
    {
        ...
        public static bool ContainsObscenities(params string[] words)
        {
            ...
            {
                // clean word
                string cleanedWord = word.CleanWord();

                // Checking bad words
                var badWords = BadWords.ToLower().Split(",");
                //.Contains(word.ToLower())
                if (badWords.Any(w => w == cleanedWord.ToLower()))
                {
                    return true;
                }

                // Checking emojis
                var badEmojis = BadEmojis.Split(" ");
                //.Contains(word)
                if (badEmojis.Any(e => e == cleanedWord))
                {
                    return true;
                }
            }
        ...
        private static string CleanWord(this string word)
        {
            // Iterate through 
            foreach (string specialCharacter in SpecialCharacters.Split(" "))
                // Has special characters
                if (word.Contains(specialCharacter))
                    // Replace with empty
                    word = word.Replace(specialCharacter, string.Empty);

            return word;
        }

        #region GitHubSource-BadWords
        /*
         * This is where I found the list of curse words
         * 
         * Link: https://github.com/dsojevic/profanity-list
         * Author: dsojevic
         * 
         */
        #endregion

        #region GitHubSource-SpecialCharacters
        /*
         * Author: redstonedesigner
         * Link: https://github.com/danielmiessler/SecLists/blob/master/Fuzzing/special-chars.txt
         */
        #endregion
    }
